...
async function scrapeData(url, tienda, rowData) {
  ...
  try {
    // Minimizar la ventana automáticamente (aparecerá en la barra de tareas)
    await page.evaluate(() => {
      window.moveTo(-2000, -2000); // Mover fuera de la pantalla visible
    });
...
async function main() {
  ...
  const sheet = workbook.Sheets['Softlines'];
  ...
  XLSX.writeFile(newWorkbook, 'resultados softlines.xlsx');
 
  console.log(`
    Proceso completado:
    - Archivo generado: resultados softlines.xlsx
    - Matches: ${filasConPriceIndex}
